...
function App() {
    ...
    function removeTask(id: string, todolistId: number) {
        todo[todolistId] = {...todo[todolistId], tasks: todo[todolistId].tasks.filter(t => t.taskId !== id)};
        setTodo([...todo]);
    }

    function addTask(title: string, todolistId: number) {
        let task = {taskId: v1(), title: title, isDone: false};
        todo[todolistId] = {...todo[todolistId], tasks: [task, ...todo[todolistId].tasks]}
        setTodo([...todo]);
    }

    function changeStatus(id: string, isDone: boolean, todolistId: number) {
        todo[todolistId] = {...todo[todolistId], tasks: todo[todolistId].tasks.map(t => t.taskId === id ? {...t, isDone} : t)};
        setTodo([...todo]);
    }

    function changeFilter(value: FilterValuesType, todolistId: number) {
        todo[todolistId] = {...todo[todolistId], filter: value};
        setTodo([...todo]);
    }

    function removeTodolist(id: number) {
        setTodo(todo.filter((item, ind) => ind !== id));
    }
